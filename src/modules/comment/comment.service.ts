...
export class CommentService {
    ...
    async createComment(newCommentDto: INewComment, user: IUser) {
        try {
            const { content,
                blog_id,
                blog_user_id
    ...
    async getComments(newPage:number, newLimit:number, id:string) {
    ...
    async replyComment(user: IUser, replyCommentDto: INewComment) {
        try {
    ...
    async updateComment(user:IUser, id:string, content:string) {
        try {
            const comment:IComment = await this.commentModel.findOneAndUpdate(
                {
                    _id: id,
                    user: user._id
                },
    ...
    async deleteComment(user: IUser, id: string) {
        try {
            const comment:IComment = await this.commentModel.findOneAndDelete({
                _id: id,
                $or: [
                    { user: user._id },
